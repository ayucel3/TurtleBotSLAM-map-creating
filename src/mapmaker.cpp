...
enum State { WALL_SEARCH, FOUND_WALL, FOLLOW_WALL};

#if DEBUG
#define CHANGE_STATE(newState) do { \
    state = newState; \
    std::cout << "STATE_CHANGE New state: " << #newState << std::endl; \
  } while(0)
#else
...
class TurtlebotCommander {
  ...
  ros::Subscriber turtlebot_scan;
  sensor_msgs::LaserScan data;
  State state;
public:
  void processData();
  void callback(const sensor_msgs::LaserScan::ConstPtr &laser_msg) {
  ...
  TurtlebotCommander() : turtlebot_pub(nh.advertise<geometry_msgs::Twist>("/cmd_vel", 1000)),
			 turtlebot_scan(nh.subscribe("/scan", 10, &TurtlebotCommander::callback, this)),
			 state(WALL_SEARCH) {
#if DEBUG
    std::cout << "STATE_CHANGE Start state: " << state << std::endl;
#endif
  }
};
...
// #endif
  ...
  return (equal(factor*HUG_DIST, data.ranges[0]) || (data.ranges[0] < factor*HUG_DIST)) && (get_front_angle() > RAD_FROM_DEG(75));
}
...
void TurtlebotCommander::processData() {
    geometry_msgs::Twist msg;
    msg.linear.y = 0.0;
    msg.linear.z = 0.0;
    msg.angular.x = 0.0;
    msg.angular.y = 0.0;
    if(WALL_SEARCH == state) {
      if(wall_on_right()) {
	...
	msg.angular.z = 0.0;
	turtlebot_pub.publish(msg);
      }
    }
    else if(FOUND_WALL == state) {
      if(parallel_to_wall()) {
        CHANGE_STATE(FOLLOW_WALL);
      }
      else if(wall_in_front()) {
	...
	msg.angular.z = get_right_angle();
      ...
      if(wall_in_front(2) || wall_on_right(1.5)) {
	msg.linear.x = linear_speed;
	turtlebot_pub.publish(msg);
      }
      else {
...
#if DEBUG
  std::cout << "Starting with debug statements turned on" << std::endl;
#else
  std::cout << "Starting with debug statements turned off" << std::endl;
#endif
  TurtlebotCommander tc;
  ros::spin();
}	
